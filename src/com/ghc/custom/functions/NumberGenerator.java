 ...
 * Example:
 * <p/>
 * NumberGenerator( 6, c:\\temp\\myFile.txt )
 * returns:
 * 000001
 * then:
 * 000002
 * and increments on subsequent executions.
 * <p/>
 */
public class NumberGenerator extends Function {
    ...
    public synchronized Object evaluate(Object data) {
        ...
        if(filename.equals(""))
            filename = "c:\\\\numberGenerator.tmp";
        System.out.println((new StringBuilder("Output to file: ")).append(filename).toString());
        ...
        prev++;
        for(newNumber = new String((new StringBuilder()).append(prev).toString()); newNumber.length() < length; newNumber = "0".concat(newNumber));
        buff.clear();
    ...
    {
        if(params.size() > 1)
            return new NumberGenerator((Function)params.get(0), (Function)params.get(1));
        else
